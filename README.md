...
The initialization of the viewer is done with a Javascript only, with a single ``main.js``. All the HTML markup required is generated by OpenFairViewer. The minimal configuration is done by specifying the OGC Catalogue Service for the Web (CSW) endpoint URL.
