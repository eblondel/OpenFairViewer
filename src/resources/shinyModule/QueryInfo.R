...
getColumnDefinitions = function(fc) {
  do.call("rbind",lapply(fc$featureType[[1]]$carrierOfCharacteristics,function(x){
	data.frame(
		MemberCode=ifelse(!is.null(x$code),x$code,""),
...
QueryInfo <- function(input, output, session) {
  ...
  observe({
	...
	#parameters (depending on the data strategy)
	...
	if(!is.null(layer)&(!is.null(wfs_server)|!is.null(wms_server))&!is.null(strategy)){
		...
		if(shiny_type=="popup"){
			...
			if(!is.null(par)){
				Data <- switch(strategy,
					"ogc_filters" = Layer$getFeatureInfo(
						srs = srs,x = x, y = y, width = width, height = height, 
						feature_count = 1000000, info_format = "application/json", 
						bbox = bbox,cql_filter = gsub(" ", "%20", gsub("''", "%27%27", URLencode(par))),
						propertyName = propertyName
					),
					"ogc_viewparams" = Layer$getFeatureInfo(
						srs = srs, x = x, y = y, width = width, height = height, 
						feature_count = 1000000, info_format = "application/json", 
						bbox = bbox, viewparams = gsub(";", "%3B", URLencode(par)),
						propertyName = propertyName
					)
				)
			}
		...
		if(shiny_type=="dashboard"){
			...
			if(!is.null(par)){
				Data <- switch(strategy,
					"ogc_filters" = ft$getFeatures(
						outputFormat = wfs_format, 
						propertyName = propertyName,
						cql_filter = gsub(" ", "%20", gsub("''", "%27%27", URLencode(par)))
					),
					"ogc_viewparams" = ft$getFeatures(
						outputFormat = wfs_format, 
						propertyName = propertyName,
						viewparams = gsub(";", "%3B", URLencode(par))
					)
				)
			}
